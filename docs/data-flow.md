...
3. Each enabled job performs checkout, installer validation, runtime setup, dependency install, and the job-specific command. `uv` jobs use job-specific cache suffixes so parallel lint, invariant, and test jobs do not try to save the same cache key.
